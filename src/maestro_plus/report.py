...
<span class="badge">{html.escape(_ATTRIBUTION_LABEL.get(diagnosis.attribution, diagnosis.attribution))}</span>
...
<dl class="meta">
  ...
  <dt>Duration</dt><dd>{html.escape(f"{diagnosis.duration_s:.1f}s" if diagnosis.duration_s else "n/a")}</dd>
...
<h2>Reproduce</h2>
<pre>maestro test {html.escape(diagnosis.flow)}{f" --device {html.escape(diagnosis.device)}" if diagnosis.device else ""}</pre>
